...
namespace BetterCRM.DataAccess.Repositories
{
    public class WorkSessionRepository : Repository<WorkSession>, IWorkSessionRepository
    {
        ...
        public async Task<WorkSession?> GetActiveSessionAsync(Guid userId)
            => await dbSet.FirstOrDefaultAsync(ws => ws.UserId == userId && ws.EndedAt == null);
        public async Task<List<WorkSession>> GetByUserAsync(Guid userId, DateTime? from = null, DateTime? to = null)
        {
            var q = dbSet.Where(ws => ws.UserId == userId && ws.EndedAt.HasValue);
            if (from.HasValue) 
        ...
        public async Task<decimal> GetTotalHoursAsync(Guid userId, DateTime from, DateTime to)
            => await dbSet.Where(ws => ws.UserId == userId && ws.EndedAt.HasValue && ws.StartedAt >= from && ws.StartedAt <= to).SumAsync(ws => (decimal)(ws.EndedAt!.Value - ws.StartedAt).TotalHours);
        public async Task<Dictionary<DateTime, decimal>> GetHoursByDayAsync(Guid userId, DateTime from, DateTime to)
            => await dbSet.Where(ws => ws.UserId == userId && ws.EndedAt.HasValue && ws.StartedAt >= from && ws.StartedAt <= to).GroupBy(ws => ws.StartedAt.Date).ToDictionaryAsync(g => g.Key, g => g.Sum(ws => (decimal)(ws.EndedAt!.Value - ws.StartedAt).TotalHours));
        public async Task<List<WorkSession>> GetByShiftAsync(Guid shiftId)
            => await dbSet.Where(ws => ws.ShiftId == shiftId && ws.EndedAt.HasValue).ToListAsync();
    }
}
