...
namespace BetterCRM.Api.Services
{
    public class ChatNotifier : IChatNotifier
    {
        ...
        public async Task SendPrivateMessageAsync(object payload, Guid recipientId) =>
            await _hub.Clients.User(recipientId.ToString()).SendAsync("ReceiveMessage", payload);

        public async Task SendDepartmentMessageAsync(object payload, Guid departmentId) =>
